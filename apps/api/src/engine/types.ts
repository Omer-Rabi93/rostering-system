...
/**
 * Date-specific worker availability (Availability v2): every calendar date the worker has an
 * entry for maps to the non-empty subset of shifts they can work THAT exact date. A date with no
 * key is the "unavailable that date" state — absence, not an empty array, is how it's represented
 * (mirrors `packages/shared`'s `monthAvailabilitySchema` sparsity rule).
 *
 ...
 * `undefined` explicitly since it's the real "unavailable" signal, not a value to paper over with
 * `!`. The JSON wire shape sent to the Python solver (`engine/problem.ts`) is a *different*, plain
 * `Record<string, readonly ShiftType[]>` — Maps don't serialize through `JSON.stringify` — built
 * only at that final boundary.
 */
 ...
 *
 * A worker is available for a given (date, shift) slot iff `availability.get(date)` is defined
 * AND contains that shift — the edit's exact calendar date, not a weekday/day-set. */

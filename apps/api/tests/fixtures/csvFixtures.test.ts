...
// `CsvImportService`/`AvailabilityService` (and, for the two file-level-error/route-only
// fixtures, `parseWorkersCsv`/`parseAvailabilityCsv`/the real HTTP route). Not every fixture is
// exercised here -- the fixtures directory is a shared resource also meant for the `loadtest/`
// scripts and manual QA (see the v4 design doc, Part C) -- but enough are covered to prove the
// fixture+expectation pairing is trustworthy, not just plausible-looking JSON nobody ever ran.
...
import { AvailabilityCsvHeaderError, CsvRowShapeError, parseAvailabilityCsv, parseWorkersCsv } from '../../src/csv/index.js';
import { CsvHeaderError } from '../../src/csv/index.js';
...
describe('CSV fixtures (apps/api/tests/fixtures/csv/)', () => {
  ...
  beforeAll(async () => {
    ...
    csvImportService = new CsvImportService(prisma, boss);
    availabilityService = new AvailabilityService(prisma, boss);
    app = buildTestApp();
  });
  ...
  beforeEach(async () => {
    ...
    // identity sequence (companyId included) -- so a `csv-import`/`availability-import` job a
    // PREVIOUS run left sitting `created` (never picked up) can collide, via its `singletonKey`
    // (`<companyId>:<kind>`), with a fresh company that happens to reuse the same id this run.
    // Mirrors `tests/routes/importExport.test.ts`'s identical convention.
  ...
  it('bad-checksum: per-row error on the nationalId field, worker not created', async () => {
    ...
    const result = await csvImportService.importCsv(csv, company);
  ...
  it('wrong-column-count-too-few: file-level CsvRowShapeError, never reaches importCsv', () => {
    const { csv, expected } = loadFixture('wrong-column-count-too-few');
    expect(expected.fileError.type).toBe('CsvRowShapeError');

    expect(() => parseWorkersCsv(csv)).toThrow(CsvRowShapeError);
    expect(() => parseWorkersCsv(csv)).toThrow(/has 6 fields, expected 7/);
  });

  it('empty-file: file-level CsvHeaderError', () => {
    const { csv, expected } = loadFixture('empty-file');
    expect(expected.fileError.type).toBe('CsvHeaderError');

    expect(() => parseWorkersCsv(csv)).toThrow(CsvHeaderError);
    expect(() => parseWorkersCsv(csv)).toThrow(/empty/);
  });
  ...
  it('header-only: zero data rows, importCsv completes with an all-zero COMPLETED task', async () => {
    ...
    const task = await prisma.importTask.findFirstOrThrow({ where: { companyId: company, kind: 'WORKER_SYNC' } });
  ...
  it('cross-company-conflict: rejected as a per-row error, never reassigns the worker', async () => {
    ...
    const result = await csvImportService.importCsv(csv, thisCompany);
  ...
  it('availability-malformed-cell: per-row AvailabilityCsvCellError on the dNN field', async () => {
    ...
    const result = await availabilityService.importCsv(csv, expected.month, company);
  ...
  it('availability-wrong-month-header: file-level AvailabilityCsvHeaderError', () => {
    const { csv, expected } = loadFixture('availability-wrong-month-header');
    expect(expected.fileError.type).toBe('AvailabilityCsvHeaderError');

    expect(() => parseAvailabilityCsv(csv, '2027-02')).toThrow(AvailabilityCsvHeaderError);
    expect(() => parseAvailabilityCsv(csv, '2027-02')).toThrow(/header for month 2027-02 must be exactly/);
  });

  it('max-rows: exactly MAX_ROWS is accepted by the real route (202)', async () => {
  ...
  it('max-rows-plus-one: MAX_ROWS + 1 is rejected by the real route (400), never enqueued', async () => {
    ...
    expect(response.status).toBe(expected.expectRoute.status);
    expect(response.body.errors?.[0]?.message ?? JSON.stringify(response.body)).toContain('exceeding the 10000-row limit');

    const task = await prisma.importTask.findFirst({ where: { companyId: company, kind: 'WORKER_SYNC' } });

...
**Git state**: real history exists (baseline commit + per-feature merges: per-company rostering, v4 import tasks, availability v3 exclusion semantics, combined workforce CSV, solver time budgets) — `git log` is now a reliable source for implementation history. The design docs under `docs/` lag the code in places; when they disagree, the code and `git log` win.
...
- **Route-scoped body size limits**: global `express.json({limit: '100kb'})` in `apps/api/src/app.ts`, except `PUT /api/availability/:month` which needs its own wider **12mb** limit (dense month-of-availability payloads for the supported 1,000–10,000-worker-per-company file sizes). Router mount *order* in `app.ts` is what makes this work — check the existing comment there before reordering routers.
- **Scale posture (two different ceilings — don't conflate them)**: I/O paths (CSV import/export, availability PUT, nginx `client_max_body_size 16m`, UI row virtualization) are sized for up to ~10,000 workers per company. The *solver* is exact-only by explicit direction (single-phase CP-SAT, min/max hours provably optimal — the approximate three-stage pipeline was reverted) and uses banded time budgets (30s → 30min by workforce size); above ~1,000 workers it may return UNKNOWN with no roster rather than an approximate one. That asymmetry is deliberate: data management must never fail at scale, roster generation prefers failing loudly over silently degrading optimality.
